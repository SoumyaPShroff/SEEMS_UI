...
const getNumericField = <T extends Record<string, unknown>>(
  ...
  // 1. First, find all keys that match case-insensitively
  const matchingKeys = keys.filter((k) => k.toLowerCase() === lowerField);
  
  // 2. Prioritize a match that has a non-zero value
  for (const key of matchingKeys) {
    const raw = obj[key];
    if (raw !== undefined && raw !== null) {
      const value = Number(String(raw).replace(/,/g, "")); // Handle strings with commas
      if (Number.isFinite(value) && value !== 0) return value;
    }
  }

  // 3. If we only found 0 or nulls, return 0 for the first matching key found
  if (matchingKeys.length > 0) {
    const val = Number(String(obj[matchingKeys[0]]).replace(/,/g, ""));
...
const buildSummaryFromData = (data: BillingData[], libWorkedJobs: LibWorkedJobsSummary[] = []) => {
  ...
  libWorkedJobs.forEach((summaryRow) => {
    libWorkedAdjustmentRules.forEach((rule) => {
      ...
      // Console log for CAM fields even if amount is 0/undefined to debug data structure
      if (rule.field.includes("CAM")) {
        console.log(`Debug CAM -> Rule: ${rule.field}, Amount Found: ${amount}, Actual Keys:`, Object.keys(summaryRow as object), "Data:", summaryRow);
      }
...
// ----------------   live code without lib,va hrs addons-------------
// import React, { useCallback, useEffect, useMemo, useRef, useState } from "react";
// import { Box, Button, CircularProgress } from "@mui/material";
// import { useNavigate } from "react-router-dom";
// import { useBillingData } from './billplanhooks/useBillingData';
